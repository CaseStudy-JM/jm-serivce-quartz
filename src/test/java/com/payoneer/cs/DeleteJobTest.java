...
class DeleteJobTest {
	...
	@Test
	void testDeleteJob() throws ExecutionException, InterruptedException {
		webTestClient.mutate().responseTimeout(Duration.ofSeconds(30)).build().delete()
				.uri("/jms/api/v1/job/".concat(jobId)).exchange().expectStatus().isOk().returnResult(Boolean.class);
	}
